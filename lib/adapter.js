...
module.exports = {
  ...
  picklists: function(connectionName, collectionName, name, cb) {
    return connections[connectionName]
      ...
      .then(function (meta) {
        if (name) {
          assert(meta.picklists[name], collectionName + '.' + name + ' is not a picklist');
          return meta.picklists[name];
        }
        return meta.picklists;
      });
  },
  ...
  retrieveFull: function(connectionName, collectionName, values, cb) {
    ...
    return this.native(connectionName, collectionName)
      ...
      .then(function (model) {
        ...
        assign(values, self._transformer.unserialize(model));
      })
      .nodeify(cb);
  }
};
