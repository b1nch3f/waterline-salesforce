  ...
  update: function (connectionName, collectionName, options, values, cb) {
    connections[connectionName]
      ...
      .then(function (connection) {
        return connection
          .sobject(collectionName)
          .update(_.extend(options.where, values));
      })
      .then(errorNet)
      .nodeify(cb);
  },
  ...
  join: function (connectionName, collectionName, options, cb) {
    ...
    function remapFields(row, table, joins) {
      joins.forEach(function (join) {
        var fieldMeta = table[join.parent].children[join.child];
        row[join.alias] = row[fieldMeta] ? row[fieldMeta].records : [];
        delete row[fieldMeta];
        ...
        if (table.sub) {
          row[join.alias] = row[join.alias].map(function (subRow) {
            // TODO support subjoins
            return remapFields(subRow, null, join.criteria.joins);
          });
        }
      });
      return row;
